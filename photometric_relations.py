'''
18-04-18: Updated the extinction maps.
'''
...
def retrieve_catalogs(starname, vizier_object, use_coords = False, RA = None, DEC = None):
    ...
    if use_coords:
        ...
        result = search_catalog_coords(RA, DEC, vizier_object, result, "I/345/gaia2")
        ...
        result = search_catalog_name(starname, vizier_object, result, "I/345/gaia2")
    ...
    try:
        ...
        if len(result) != 0:
            ...
            if "II/246/out" in name_cats:
                ...
                if len(result[i][0]['Jmag']) > 1:
                    quality = result[i][0]['Qflg']

                    nA = np.zeros(len(quality))
                    for k in range(len(quality)):
                        nA[k] = quality[k].count('A')

                    maxA = np.argmax(nA)

                    jmag = result[i][0]['Jmag'][maxA]
                    kmag = result[i][0]['Kmag'][maxA]
                    hmag = result[i][0]['Hmag'][maxA]
                    e_jmag = result[i][0]['e_Jmag'][maxA]
                    e_kmag = result[i][0]['e_Kmag'][maxA]
                    e_hmag = result[i][0]['e_Hmag'][maxA]

                    photometry['RA'] = [result[i][0]['RAJ2000'][maxA], "II/246/out"]
                    photometry['DEC'] = [result[i][0]['DEJ2000'][maxA], "II/246/out"]
                ...
                if np.isnan(float(jmag)) == False:
                    photometry['J'] = [jmag, f_errors[0], "II/246/out", '2MASS J', 0.0]
                if np.isnan(float(kmag)) == False:
                    photometry['K'] = [kmag, f_errors[1], "II/246/out", '2MASS Ks', 0.0]
                if np.isnan(float(hmag)) == False:
                    photometry['H'] = [hmag, f_errors[2], "II/246/out", '2MASS H', 0.0]

            # GAIA DR2
            if "I/345/gaia2" in name_cats:
                i = name_cats.index("I/345/gaia2")
                p_teff = result[i][0]['Teff']
                i2 = np.where(p_teff > 0.0)[0]
                if len(result[i][0]['Plx'][i2]) >= 1:
                    if len(result[i][0]['Plx'][i2]) > 1:
                        plx0 = result[i][0]['Plx'][i2]
                        i0 = np.where(plx0 > 0.0)[0]
                        if len(i0) > 1:
                            i0 = 0
                        else:
                            i0 = int(i0)
                        ...
                        i0 = 0
                    plx = result[i][0]['Plx'][i2][i0]
                    e_plx = result[i][0]['e_Plx'][i2][i0]
                    photometry['parallax'] = [plx*u.mas, e_plx*u.mas, "I/345/gaia2"]
                    photometry['RA'] = [result[i][0]['RA_ICRS'][i2][i0], "I/345/gaia2"]
                    photometry['DEC'] = [result[i][0]['DE_ICRS'][i2][i0], "I/345/gaia2"]
                else:
                    plx = result[i][0]['Plx'][0]
                    e_plx = result[i][0]['e_Plx'][0]
                    photometry['parallax'] = [plx*u.mas, e_plx*u.mas, "I/345/gaia2"]
                    photometry['RA'] = [result[i][0]['RA_ICRS'][0], "I/345/gaia2"]
                    photometry['DEC'] = [result[i][0]['DE_ICRS'][0], "I/345/gaia2"]
            ...
            if ("I/259/tyc2" in name_cats):
                ...
                if np.isnan(float(bmag)) == False:
                    photometry['Bt'] = [bmag, e_bmag, "I/259/tyc2", 'HIPPARCOS BT', 0.0]
                if np.isnan(float(vmag)) == False:
                    photometry['Vt'] = [vmag, e_vmag, "I/259/tyc2", 'HIPPARCOS VT', 0.0]
            ...
            if "J/MNRAS/403/1949/ubvri" in name_cats:
                ...
                if np.isnan(float(vmag)) == False:
                    photometry['V'] = [vmag, e_hpmag, "J/MNRAS/403/1949/ubvri", 'CTIO V', 0.0]
                if np.isnan(float(BV + vmag)) == False:
                    photometry['B'] = [BV + vmag, e_hpmag, "J/MNRAS/403/1949/ubvri", 'CTIO B', 0.0]
                if np.isnan(float(vmag - VR)) == False:
                    photometry['R'] = [vmag - VR, e_hpmag, "J/MNRAS/403/1949/ubvri", 'CTIO R', 0.0]
                if np.isnan(float(vmag - VI)) == False:
                    photometry['I'] = [vmag - VI, e_hpmag, "J/MNRAS/403/1949/ubvri", 'CTIO I', 0.0]
                ...
                if "J/MNRAS/373/13/table1" in name_cats:
                    ...
                    if np.isnan(float(vmag)) == False:
                        photometry['V'] = [vmag, e_hpmag, "J/MNRAS/373/13/table1", 'CTIO V', 0.0]
                    if np.isnan(float(BV + vmag)) == False:
                        photometry['B'] = [BV + vmag, e_hpmag, "J/MNRAS/373/13/table1", 'CTIO B', 0.0]
                    if np.isnan(float(vmag - VR)) == False:
                        photometry['R'] = [vmag - VR, e_hpmag, "J/MNRAS/373/13/table1", 'CTIO R', 0.0]
                    if np.isnan(float(vmag - VR - RI)) == False:
                        photometry['I'] = [vmag - VR - RI, e_hpmag, "J/MNRAS/373/13/table1", 'CTIO I', 0.0]
                    if np.isnan(float(J)) == False:
                        photometry['J'] = [J, e_J, "J/MNRAS/373/13/table1", '2MASS J', 0.0]
                    if np.isnan(float(H)) == False:
                        photometry['H'] = [H, e_H, "J/MNRAS/373/13/table1", '2MASS H', 0.0]
                    if np.isnan(float(K)) == False:
                        photometry['K'] = [K, e_K, "J/MNRAS/373/13/table1", '2MASS Ks', 0.0]
                ...
                # Beers et al. 2007
                    ...
                    if e_K == 0.0: e_k = 0.01
                    if np.isnan(float(vmag)) == False:
                        photometry['V'] = [vmag, e_vmag, "J/ApJS/168/128/survey", 'CTIO V', 0.0]
                    if np.isnan(float(BV + vmag)) == False:
                        photometry['B'] = [BV + vmag, np.sqrt(e_vmag**2. + e_BV**2.), "J/ApJS/168/128/survey", 'CTIO B', 0.0]
                    if np.isnan(float(vmag - VR)) == False:
                        photometry['R'] = [vmag - VR, np.sqrt(e_vmag**2. + e_VR**2.), "J/ApJS/168/128/survey", 'CTIO R', 0.0]
                    if np.isnan(float(vmag - VI)) == False:
                        photometry['I'] = [vmag - VI, np.sqrt(e_vmag**2. + e_VI**2.), "J/ApJS/168/128/survey", 'CTIO I', 0.0]
                    if np.isnan(float(J)) == False:
                        photometry['J'] = [J, e_J, "J/ApJS/168/128/survey", '2MASS J', 0.0]
                    if np.isnan(float(H)) == False:
                        photometry['H'] = [H, e_H, "J/ApJS/168/128/survey", '2MASS H', 0.0]
                    if np.isnan(float(K)) == False:
                        photometry['K'] = [K, e_K, "J/ApJS/168/128/survey", '2MASS Ks', 0.0]
            ...
            if "II/215/catalog" in name_cats:
                ...
                if ('V' in photometry.keys()) == False:
                    if np.isnan(float(vmag)) == False:
                        photometry['V'] = [vmag, e_vmag, "II/215/catalog", 'CTIO V', 0.0]
                if np.isnan(float(by)) == False and np.isnan(float(e_by)) == False:
                    photometry['b'] = [by, e_by, "II/215/catalog", 'Stromgren b', 0.0]
                    photometry['y'] = [0., e_by, "II/215/catalog", 'Stromgren y', 0.0]
                ...
                if "V/130/gcs3" in name_cats:
                    ...
                    if ('V' in photometry.keys()) == False:
                        if np.isnan(float(vmag)) == False:
                            photometry['V'] = [vmag, e_hpmag, "V/130/gcs3", 'CTIO V', 0.0]
                    if np.isnan(float(by)) == False:
                        photometry['b'] = [by, e_hpmag, "V/130/gcs3", 'Stromgren b', 0.0]
                        photometry['y'] = [0., e_hpmag, "V/130/gcs3", 'Stromgren y', 0.0]
            ...
            if (('R' in photo_keys) == False and ('I' in photo_keys) == False and \
                ...
                if "II/237/colors" in name_cats:
                    ...
                    if np.isnan(float(UV + vmag)) == False:
                        photometry['U'] = [UV + vmag, e_hpmag, "II/237/colors", 'CTIO U', 0.0]
                    if np.isnan(float(vmag)) == False:
                        photometry['V'] = [vmag, e_hpmag, "II/237/colors", 'CTIO V', 0.0]
                    if np.isnan(float(BV + vmag)) == False:
                        photometry['B'] = [BV + vmag, e_hpmag, "II/237/colors", 'CTIO B', 0.0]
                    if np.isnan(float(RV + vmag)) == False:
                        photometry['R'] = [RV + vmag, e_hpmag, "II/237/colors", 'CTIO R', 0.0]
                    if np.isnan(float(IV + vmag)) == False:
                        photometry['I'] = [IV + vmag, e_hpmag, "II/237/colors", 'CTIO I', 0.0]
                       ...
                       (('K' in photo_keys) and (photometry['K'][1] > 1.0)):
                        if np.isnan(float(JV + vmag)) == False:
                            if np.isnan(float(e_J)) == False:
                                photometry['J'] = [JV + vmag, e_J*1e-2, "II/237/colors", '2MASS J', 0.0]
                            else:
                                photometry['J'] = [JV + vmag, e_hpmag, "II/237/colors", '2MASS J', 0.0]
                        if np.isnan(float(HV + vmag)) == False:
                            if np.isnan(float(e_H)) == False:
                                photometry['H'] = [HV + vmag, e_H*1e-2, "II/237/colors", '2MASS H', 0.0]
                            else:
                                photometry['H'] = [HV + vmag, e_hpmag, "II/237/colors", '2MASS H', 0.0]
                        if np.isnan(float(KV + vmag)) == False:
                            if np.isnan(float(e_K)) == False:
                                photometry['K'] = [KV + vmag, e_K*1e-2, "II/237/colors", '2MASS Ks', 0.0]
                            else:
                                photometry['K'] = [KV + vmag, e_hpmag, "II/237/colors", '2MASS Ks', 0.0]
                    ...
                    if (np.isnan(float(vmag)) == False) and (np.isnan(float(e_vmag)) == False) and (float(e_vmag) > 0.0):
                        photometry['V'] = [vmag, e_vmag, "II/336/apass9", 'CTIO V', 0.0]
                    if (np.isnan(float(bmag)) == False) and (np.isnan(float(e_bmag)) == False) and (float(e_bmag) > 0.0):
                        photometry['B'] = [bmag, e_bmag, "II/336/apass9", 'CTIO B', 0.0]
...
def correct_extinction(photometry):
    ...
    if ('RA' in k) and ('DEC' in k):
        ...
        if 'parallax' in k:
            ...
            d = p.to(u.parsec, equivalencies=u.parallax()).value/1000. # in Kpc
        ...
        for m in mag:
            if m in k:
                wave = photo[m][3]
                if (wave == 'HIPPARCOS BT') or (wave == 'HIPPARCOS VT'):
                    # Obtain the extinction in BV

                    ext_map._filter = 'CTIO B'
                    A_B = ext_map(l,b,d)
                    ext_map._filter = 'CTIO V'
                    A_V = ext_map(l,b,d)
