...
def make_cmd_dist_plots(starname, mass, age, logg, T, logg_s, radius, feh, err_T, err_logg_s):
    ...
    dar = MIST_Isochrone()
    masses = np.arange(mass_p[1] - 0.5, mass_p[1] + 0.5, 0.05)
    for m in masses:
        ev = dar.evtrack(m, feh = feh, dage = 0.05)
    ...
    ev = dar.evtrack(mass_p[1], feh = feh, dage = 0.05)
    ...
    del dar, ev, masses, fig, ax1, ax2, ax3, ax4, mass_p, age_p, logg_p, radius_p, imass, iage, iradius, new_mass, new_age, new_radius
